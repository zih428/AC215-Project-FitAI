...
- **CD (GitHub Actions):** Push to `main` (or manually trigger the “CD” workflow) to build all service images, push to Artifact Registry, update Pulumi config with new tags, and run `pulumi up` against stack `FitAI_infra/dev`. Required GitHub secrets: `PULUMI_ACCESS_TOKEN`, `GCP_CREDENTIALS_JSON` (service account JSON with Artifact Registry + GKE perms), optional `PULUMI_CONFIG_PASSPHRASE` and any Pulumi config secrets (`dbPassword`, `jwtSecret`, `gcpServiceAccountKey`, etc.).
